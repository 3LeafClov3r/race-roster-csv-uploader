...
class Information:
    ...
    def get_current_df(self):
        ...
        for i in range(len(self.dataframe)):
            start_date_i = self.dataframe['Registration start date (YYYY-MM-DD) (inclusive)'].iloc[i]
            start_date = datetime.strptime(start_date_i, "%Y-%m-%d")
            end_date_i = self.dataframe['Registration end date (YYYY-MM-DD) (inclusive)'].iloc[i]
            end_date = datetime.strptime(end_date_i, "%Y-%m-%d")
            if start_date <= today and end_date >= today:
                return self.get_dataframe_by_gid(self.dataframe['sheet gid'].iloc[i])
            
        return None
    
    def get_gid_current_year(self) -> str:
        today = datetime.today()
        for i in range(len(self.dataframe)):
            start_date_i = self.dataframe['Registration start date (YYYY-MM-DD) (inclusive)'].iloc[i]
            start_date = datetime.strptime(start_date_i, "%Y-%m-%d")
            ...
            end_date = datetime.strptime(end_date_i, "%Y-%m-%d")
            if start_date <= today and end_date >= today:
                return self.dataframe['sheet gid'].iloc[i]
        return ""
